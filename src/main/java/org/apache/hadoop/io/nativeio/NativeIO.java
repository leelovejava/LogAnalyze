...
public class NativeIO {
	public static class POSIX {
			...
			if (NativeCodeLoader.isNativeCodeLoaded()) {
				try {
					...
					PerformanceAdvisory.LOG.debug(
							"Unable to initialize NativeIO libraries", t);
				}
			}
		}
		...
		public static native FileDescriptor open(String path, int flags,
			...
			if (!Shell.WINDOWS) {
				...
				try {
					...
					if (nioe.getErrorCode() == 3) {
						throw new NativeIOException(
								"No such file or directory", Errno.ENOENT);
					} else {
						LOG.warn(String.format("NativeIO.chmod error (%d): %s",
								nioe.getErrorCode(), nioe.getMessage()));
						throw new NativeIOException("Unknown error",
								Errno.UNKNOWN);
					}
				}
			}
		...
		static native void posix_fadvise(FileDescriptor fd, long offset,
				long len, int flags) throws NativeIOException;

		/** Wrapper around sync_file_range(2) */
		static native void sync_file_range(FileDescriptor fd, long offset,
				long nbytes, int flags) throws NativeIOException;
		...
		public static void syncFileRangeIfPossible(FileDescriptor fd,
				long offset, long nbytes, int flags) throws NativeIOException {
		...
		static native void mlock_native(ByteBuffer buffer, long len)
				throws NativeIOException;
		...
		public static void munmap(MappedByteBuffer buffer) {
			if (buffer instanceof sun.nio.ch.DirectBuffer) {
				sun.misc.Cleaner cleaner = ((sun.nio.ch.DirectBuffer) buffer)
						.cleaner();
				cleaner.clean();
			}
		}
		...
		private static String getName(IdCache domain, int id)
			...
			if (cachedName != null
					&& (cachedName.timestamp + cacheTimeout) > now) {
				name = cachedName.name;
			} else {
				name = (domain == IdCache.USER) ? getUserName(id)
						: getGroupName(id);
				if (LOG.isDebugEnabled()) {
					String type = (domain == IdCache.USER) ? "UserName"
							: "GroupName";
	...
	public static class Windows {
		...
		public static boolean access(String path, AccessRight desiredAccess)
				throws IOException {
			/** local mode runing begin
			 * return access0(path, desiredAccess.accessRight());
			 * local mode runing end
			 */
			return true;
		}
	...
	static long getOperatingSystemPageSize() {
		try {
			...
			LOG.warn(
					"Unable to get operating system page size.  Guessing 4096.",
					e);
			return 4096;
		}
	}
	...
	public static FileInputStream getShareDeleteFileInputStream(File f)
		...
		if (!Shell.WINDOWS) {
			...
			//
			FileDescriptor fd = Windows.createFile(f.getAbsolutePath(),
					Windows.GENERIC_READ, Windows.FILE_SHARE_READ
							| Windows.FILE_SHARE_WRITE
							| Windows.FILE_SHARE_DELETE, Windows.OPEN_EXISTING);
	...
	public static FileInputStream getShareDeleteFileInputStream(File f,
		...
		if (!Shell.WINDOWS) {
			...
			FileDescriptor fd = Windows.createFile(
					f.getAbsolutePath(), Windows.GENERIC_READ,
					Windows.FILE_SHARE_READ
							| Windows.FILE_SHARE_WRITE
							| Windows.FILE_SHARE_DELETE,
					Windows.OPEN_EXISTING);
			if (seekOffset > 0)
				Windows.setFilePointer(fd, seekOffset,
						Windows.FILE_BEGIN);
			return new FileInputStream(fd);
		}
	}
	 ...
	 */
	public static FileOutputStream getCreateForWriteFileOutputStream(File f,
		...
		if (!Shell.WINDOWS) {
			...
			try {
				FileDescriptor fd = Windows.createFile(
						f.getCanonicalPath(), Windows.GENERIC_WRITE,
						Windows.FILE_SHARE_DELETE
								| Windows.FILE_SHARE_READ
								| Windows.FILE_SHARE_WRITE,
						Windows.CREATE_NEW);
	...
	private synchronized static void ensureInitialized() {
		if (!initialized) {
			cacheTimeout = new Configuration().getLong(
					"hadoop.security.uid.cache.secs", 4 * 60 * 60) * 1000;
	...
	public static void renameTo(File src, File dst) throws IOException {
		if (!nativeLoaded) {
			if (!src.renameTo(dst)) {
				throw new IOException("renameTo(src=" + src + ", dst=" + dst
						+ ") failed.");
			}
		} else {
	 ...
	 * @throws IOException
	 */
	public static void copyFileUnbuffered(File src, File dst)
			throws IOException {
