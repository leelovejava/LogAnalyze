...
-- 4. 创建function
create function currency_type_convert as 'com.sxt.transformer.hive.CurrencyTypeDimensionUDF' using jar 'hdfs://node1:8020/sxt/transformer/transformer-0.0.1.jar';
create function payment_type_convert as 'com.sxt.transformer.hive.PaymentTypeDimensionUDF' using jar 'hdfs://node1:8020/sxt/transformer/transformer-0.0.1.jar';
create function order_info as 'com.sxt.transformer.hive.OrderInfoUDF' using jar 'hdfs://node1:8020/sxt/transformer/transformer-0.0.1.jar';
create function order_total_amount as 'com.sxt.transformer.hive.OrderTotalAmountUDF' using jar 'hdfs://node1:8020/sxt/transformer/transformer-0.0.1.jar';

-- 5. 创建临时表
create table stats_order_tmp1(pl string, dt string, cut string, pt string, values bigint);
CREATE TABLE `stats_order_tmp2` (`platform_dimension_id` bigint ,`date_dimension_id` bigint , `currency_type_dimension_id` bigint ,`payment_type_dimension_id` bigint , `values` bigint, `created` string);
